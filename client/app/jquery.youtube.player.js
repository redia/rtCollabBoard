...
(function($, window, document, undefined){
	...
	player.prototype = {
		...
		_createPlayer : function(){
			...
			var 
				...
				swfpath = 
					...
					? 'https://www.youtube.com/apiplayer?enablejsapi=1&version=3&playerapiid='+apiid+'&hd=' + this.options.highDef + '&showinfo=0'
					: 'https://www.youtube.com/v/' + id + '?enablejsapi=1&playerapiid='+apiid+'&version=3';
		...
		_createPlaylist : function(){
			...
			this._addVideosToPlaylist = function(cue){
				...
				$.each(videos, function(){
					...
					$('<li />')
						.data('video', this)
						.append( self.options.videoThumbs ? '<img alt="' + this.title + '" title="' + this.title + '" src="https://img.youtube.com/vi/' + this.id + '/2.jpg" />' : this.title)
		...
		_getPlaylistData : function(success, error){
			...
			if (playlist.user || playlist.playlist) {
				...
				var url = playlist.user 
					...
					: 'https://gdata.youtube.com/feeds/api/playlists/' + playlist.playlist;
		...
		pauseVideo : function(){
			this.options.socket.emit('stopMovie');	
		...
		muteVideo : function(button){
			if(this._state('mute')){				
				...
				this.options.socket.emit('unMuteMovie');	
			}
			else{
				this.youtubePlayer.mute();
				this.options.socket.emit('muteMovie');	
			}
		},
		...
		playVideo : function(){
			this.options.socket.emit('startMovie');	
